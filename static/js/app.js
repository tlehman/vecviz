...
function animate() {
  ...
  if (points && pointsData.length > 0) {
    ...
    if (intersects.length > 0) {
      ...
      // Map back to original pointsData index
      const originalIndex = visibleIndices[visibleIndex];
      if (originalIndex !== hoveredIndex) {
        hoveredIndex = originalIndex;
        document.getElementById("point-text").textContent =
          pointsData[originalIndex].text;
      }
    }
  }
...
// Store visible indices mapping for hover functionality
let visibleIndices = [];
...
function renderPoints() {
  ...
  for (let i = 0; i < pointsData.length; i++) {
    if (!query || pointsData[i].text.toLowerCase().includes(query)) {
  ...
  if (visibleIndices.length === 0) {
    document.getElementById("point-count").textContent =
      `0 of ${pointsData.length} points (filtered)`;
    return;
  }
  ...
  for (let j = 0; j < visibleIndices.length; j++) {
    ...
    positions[j * 3] = pointsData[i].x;
    positions[j * 3 + 1] = pointsData[i].y;
    positions[j * 3 + 2] = pointsData[i].z;
  ...
  if (query) {
    document.getElementById("point-count").textContent =
      `${visibleIndices.length} of ${pointsData.length} points`;
  } else {
    document.getElementById("point-count").textContent =
      `${pointsData.length} points`;
  }
}
...
async function refreshVisualization() {
  ...
  if (data.needs_update) {
    setStatus(
      'New embeddings added. Click "Recompute t-SNE" to update.',
      "warning",
    );
  }
}
...
document.getElementById("embed-form").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    input.value = "";
    setStatus(
      `Embedding added (${result.embedding_dim}D). Recompute t-SNE to visualize.`,
      "success",
    );
  } catch (error) {
...
document.getElementById("recompute-btn").addEventListener("click", async () => {
  ...
  try {
    ...
    setStatus(
      `t-SNE complete: ${result.points_processed} points in ${result.computation_time_ms}ms`,
      "success",
    );
